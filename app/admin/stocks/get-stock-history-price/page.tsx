...
interface HistoryState {
    stockData: StockPriceRecord[] | null;
    error: string | null;
}
...
async function fetchStockData(prevState: HistoryState, formData: FormData): Promise<HistoryState> {
    ...
    try {
        const data = await getStockHistoryPrice({ stock_no: stockNo, date });
        return { stockData: data as StockPriceRecord[], error: null };
    } catch (error) {
...
export default function Search() {
    ...
    const calculateStats = (data: StockPriceRecord[]): PriceStats | null => {
        if (!data?.length) return null;
        const highestPrice = Math.max(...data.map(item => item.close_price));
        const lowestPrice = Math.min(...data.map(item => item.close_price));
        const averagePrice = (data.reduce((sum, item) => sum + item.close_price, 0) / data.length).toFixed(2);
        ...
        return { highestPrice, lowestPrice, averagePrice, startPrice, endPrice, changeRate };
    };
        ...
        <div className="w-full lg:w-4/5 max-h-[calc(100svh-180px)] overflow-auto p-6 bg-gray-100">
            ...
            <form action={formAction} className="mb-4">
                <div className="flex flex-col md:flex-row md:space-x-2">
                    <input type="text" name="stockNo" placeholder="輸入股票代號" className="p-2 border rounded w-full mb-2 md:mb-0" />
                    <input type="text" name="date" placeholder="輸入日期 (YYYYMMDD 格式)" className="p-2 border rounded w-full mb-2 md:mb-0" />
                    <button type="submit" className="p-2 bg-blue-500 text-white rounded w-full md:w-auto md:px-4" disabled={isPending}>
                        {isPending ? "查詢中..." : "查詢"}
                    </button>
                </div>
            </form>
            {state.error && <p className="text-red-500">{state.error}</p>}
            {state.stockData && state.stockData.length > 0 && stats && (
                <div className="mt-6">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">收盤價</th>
                                ...
                                {state.stockData.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={item.date}>
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{item.close_price.toFixed(2)}</td>
            ...
            {state.stockData && state.stockData.length === 0 && (
                <div className="p-4 bg-yellow-100 text-yellow-700 rounded">沒有找到相關股票的歷史資料</div>
            )}
        </div>
    );
}
