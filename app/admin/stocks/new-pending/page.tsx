...
export default function NewPending() {
    ...
    const [stockData, setStockData] = useState<StockChange | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStockData(data as StockChange);
        } catch {
    ...
    return (
        <div className="w-full md:w-4/5 lg:w-3/5 xl:w-2/5 max-h-[calc(100svh-180px)] overflow-auto p-6 bg-gray-100">
            ...
            <form onSubmit={handleSubmit} className="mb-4">
                <input type="text" value={stockNo} onChange={(e) => setStockNo(e.target.value)} placeholder="輸入股票代號" className="p-2 border rounded w-full mb-2" />
                <input type="text" value={startDate} onChange={(e) => setStartDate(e.target.value)} placeholder="輸入起始時間 (民國年月日)" className="p-2 border rounded w-full mb-2" />
                <input type="text" value={endDate} onChange={(e) => setEndDate(e.target.value)} placeholder="輸入結束時間 (民國年月日)" className="p-2 border rounded w-full mb-2" />
                <button type="submit" className="p-2 bg-blue-500 text-white rounded w-full">排程</button>
            </form>
