...
interface SearchState {
    stockData: StockChange | null;
    error: string | null;
}

async function fetchStockData(prevState: SearchState, formData: FormData): Promise<SearchState> {
    ...
    try {
        ...
        return { stockData: data as StockChange, error: null };
    } catch {
    ...
    return (
        <div className="w-full md:w-4/5 lg:w-3/5 xl:w-2/5 max-h-[calc(100svh-180px)] overflow-auto p-6 bg-gray-100">
            ...
            <form action={formAction} className="mb-4">
                <input type="text" name="stockNo" placeholder="輸入股票代號" className="p-2 border rounded w-full mb-2" />
                <input type="text" name="startDate" placeholder="輸入起始時間 (民國年月日)" className="p-2 border rounded w-full mb-2" />
                <input type="text" name="endDate" placeholder="輸入結束時間 (民國年月日)" className="p-2 border rounded w-full mb-2" />
                <button type="submit" className="p-2 bg-blue-500 text-white rounded w-full" disabled={isPending}>{isPending ? "查詢中..." : "查詢"}</button>
            </form>
