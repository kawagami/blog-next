...
import { convertTextAction } from "@/app/(public)/tools/convert-text/actions";
    ...
    return (
        <div className="w-full h-[calc(100svh-120px)] overflow-auto flex flex-col items-center">
            ...
            <form action={formAction} className="w-4/6 flex flex-col items-center">
                <textarea name="inputText" rows={5} placeholder="請輸入簡體中文" className="w-full mb-3 p-3 text-lg border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <button type="submit" className="px-5 py-2 text-lg bg-blue-500 text-white rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 mb-3" disabled={isPending}>
                    {isPending ? "處理中..." : "轉換為繁體"}
                </button>
            </form>
            <h2 className="text-xl font-semibold mb-2">轉換結果：</h2>
            <textarea rows={5} readOnly value={state.converted_text} className="w-4/6 p-3 text-lg border border-gray-300 rounded-md bg-gray-100" />
            <button onClick={handleCopy} className="px-5 py-2 text-lg bg-green-500 text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400 mt-3">複製結果</button>
            {notification && (
