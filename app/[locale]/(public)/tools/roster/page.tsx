...
export default function RosterPage() {
    const [names, setNames] = useState<string[]>(["人員1", "人員2"]);
    ...
    const addName = () => {
        if (newName.trim()) {
            if (names.includes(newName.trim())) { alert("姓名重複了"); return; }
    ...
    const handleGenerate = async () => {
        if (names.length === 0) { alert("請至少輸入一位人員"); return; }
        setLoading(true);
        try {
    ...
    return (
        <div className="w-full max-w-6xl px-4 pb-10 space-y-8">
            <section className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md p-6 rounded-3xl shadow-lg border border-white/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                            <input value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addName()} placeholder="輸入姓名..." className="flex-1 px-4 py-2 rounded-xl border focus:ring-2 focus:ring-blue-500 outline-none dark:bg-gray-700 dark:border-gray-600" />
                            <button onClick={addName} className="p-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors"><UserPlus size={20} /></button>
                        </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="number" min="1" max="31" value={days} onChange={(e) => setDays(Math.min(31, Math.max(1, parseInt(e.target.value) || 1)))} className="w-full mt-1 px-4 py-2 rounded-xl border dark:bg-gray-700 dark:border-gray-600 outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">排班規則</label>
                            <select value={rule} onChange={(e) => setRule(e.target.value)} className="w-full mt-1 px-4 py-2 rounded-xl border dark:bg-gray-700 dark:border-gray-600 outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <button onClick={handleGenerate} disabled={loading || names.length === 0} className="w-full mt-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-all active:scale-[0.98]">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-100/50 dark:bg-gray-800/50">
                                    <th className="p-4 border-b dark:border-gray-700 font-semibold sticky left-0 bg-white/95 dark:bg-gray-800/95 z-20">人員名單</th>
                                    {rosterData[0]?.shifts.map((_, i) => (
                                        <th key={i} className="p-4 border-b dark:border-gray-700 font-semibold text-center min-w-[90px]">
                                            <div className="text-xs text-gray-500 font-mono">DAY {i + 1}</div>
                                        </th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {rosterData.map((staff) => (
                                    <tr key={staff.id} className="hover:bg-white/40 dark:hover:bg-gray-800/40 transition-colors">
                                        <td className="p-4 border-b dark:border-gray-700 font-bold sticky left-0 bg-white/95 dark:bg-gray-800/95 z-10">{staff.name}</td>
                                        {staff.shifts.map((shift, idx) => (
                                            <td key={idx} className="p-2 border-b dark:border-gray-700 text-center"><ShiftBadge type={shift} /></td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
