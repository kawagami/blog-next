...
// 後端 wire 格式（見 docs/chess-multiplayer-spec.md §3 + 後端定案）。
// 信封固定 { type, data }；color/side/winner 全小寫 "red"/"black"。

export type GameOverReason =
    | 'checkmate' | 'stalemate' | 'timeout' | 'resign' | 'disconnect' | 'draw_60';

export interface MatchFoundData { color: Side; clock_ms: number; }
...
export interface ErrorData { reason: 'not_in_game' | 'game_ended' | string; }
