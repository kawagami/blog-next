...
export default function HackmdNoteTagButtons({ tags }: { tags: HackmdTag[] }) {
    ...
    const clickAll = () => setOpenArray(tags.map(tag => tag.name));
    const clickNothing = () => setOpenArray([""]);
                ...
                <div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                        {tags.map(tag => {
                            ...
                            return (
                                <span className={`${commonStyle} ${tailwindStyle}`} key={tag.id} onClick={() => checkExist(tag.name)}>
                                    {tag.name}
                                </span>
                            );
                        })}
