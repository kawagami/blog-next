...
interface Props {
    notes: HackmdNote[];
    defaultOpenArray: string[];
}

export default function HackMDNotesComponent({ notes, defaultOpenArray }: Props) {
    ...
    useEffect(() => {
        if (openArray.length === 0) setOpenArray(defaultOpenArray);
    }, [openArray, defaultOpenArray, setOpenArray]);
